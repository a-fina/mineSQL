...
Ext.extend(Ext.grid.GridFilters, Ext.util.Observable, {
	...
	applyState: function(grid, state) {
		...
		if(state.filters) {
			for(var key in state.filters) {
				...
				if(filter) {
                    //alert(" settaggio value e activw applyState ");
		...
		if(this.local) {
            //alert(" dentro applyState prima di reload"); 
			this.reload();
    }
	...
	onRender: function(){
		...
		if(this.showMenu) {
			...
			//hmenu.on('checkchange', this.onMenu, this);
		}
	...
	onCheckChange: function(item, value) {
        var padre = this.getMenuFilter().dataIndex;
        //alert(" GridFilter Padre onCheckChange item : " + item.text + " vale: " + value + " name: " + padre+ " type: " + item.type);
		this.getMenuFilter().setActive(value);
        // ha un submenu? : " + item.menu.items);
        // Disattivo tutti i filtri selezionati e 
        var len = item.menu.items.items.length; 
        if ( ! value ){
            for(i=0;i<len;i++){
                try {
                    item.menu.items.items[i].setChecked(false,false);
                }catch(e) {
                    try { 
                        item.menu.items.items[i].setValue(""); 
                        var c = item.menu.items.items[i];
                        c.fireEvent("update", this);
                    }catch(e){}
                }
            }
        }
	...
	onStateChange: function(event, filter) {
        //alert(" entrata 1 onStateChange ");
        if(event == "serialize") {
          return;
        }
        //alert(" entrata 2 onStateChange ");
	...
	updateColumnHeadings: function(view) {
        ...
        //alert(" entra in updateColumnHeadings ");
		...
		for(var i=0, len=view.cm.config.length; i<len; i++) {
            ...
            /** try{
                console.log(" filter: " +  view.cm.config[i].dataIndex + "  active: " + filter.active );
            }catch(e){
                console.log(" filter: " + filter + " Exception: " +e);
            } **/
			if(filter && filter.active) {
                //alert("MARK OK updateColumnHeadings 3");
